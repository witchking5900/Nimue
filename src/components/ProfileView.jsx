      ...
      {!isInfiniteHearts && hearts < maxHearts && (
        <div className={`mb-6 p-6 rounded-2xl border-2 ${isMagical ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100'}`}>
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <button onClick={handleFullRestore} className={`min-h-[140px] flex flex-col justify-between p-4 rounded-xl border-2 transition-all ${isMagical ? 'border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400' : 'border-blue-200 bg-blue-50 hover:bg-blue-100 text-blue-700'}`}><div className="w-full flex justify-center"><CreditCard size={32} /></div><div className="font-bold text-sm text-center">{text.fullBtn}</div><div className="w-full flex justify-center"><span className="text-xs opacity-70 font-bold">{text.fullCost}</span></div></button>
            </div>
        </div>
      )}
