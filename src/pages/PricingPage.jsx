...
export default function PricingPage() {
  ...
  const t = {
    en: {
        ...
        gmDescMag: "Pay once. Rule forever.",
        gmDescStd: "One-time payment. No monthly fees.",
    ...
    ka: {
        ...
        gmDescMag: "გადაიხადეთ ერთხელ. იმეფეთ მარად.",
        gmDescStd: "ერთჯერადი გადახდა. ყოველთვიური გადასახადის გარეშე.",
    ...
    <div className={`min-h-screen py-20 px-4 relative overflow-hidden transition-colors duration-500 ${
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <div className="absolute -top-10 left-0">
            <button 
                onClick={() => navigate('/')}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition-all transform hover:-translate-x-1 ${
                    isMagical 
            ...
            >
                <ChevronLeft size={20} />
                {isMagical ? text.backMag : text.backStd}
            </button>
        </div>
        ...
        <div className="text-center mb-12 animate-in slide-in-from-top-4">
          ...
          <p className={`text-lg ${isMagical ? 'text-slate-400' : 'text-slate-600'}`}>
        ...
        {IS_LIFETIME_DEAL_ACTIVE && (
            <div className="mb-12 relative group animate-in zoom-in duration-500">
                ...
                <div className={`relative rounded-xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden border shadow-xl ${
                    isMagical 
                    ...
                    <div className="flex items-center gap-4">
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center border ${
                            isMagical 
                            ? 'bg-purple-900/30 text-amber-400 border-purple-500/30' 
                            : 'bg-blue-50 text-blue-600 border-blue-100'
                        }`}>
                            <Crown size={32} />
                        </div>
                        <div>
                            <h3 className={`text-2xl font-bold flex items-center gap-2 ${isMagical ? 'text-white' : 'text-slate-900'}`}>
                                {isMagical ? text.gmTitleMag : text.gmTitleStd}
                            </h3>
                            <p className={`text-sm ${isMagical ? 'text-purple-300' : 'text-slate-500'}`}>
                                {isMagical ? text.gmDescMag : text.gmDescStd}
                            </p>
                            
                            <div className="flex flex-wrap gap-2 mt-2">
                                {grandMagusFeatures.slice(0, 2).map((f, i) => (
                    ...
                    <div className="flex items-center gap-6">
                        <div className="text-right hidden md:block">
                            <div className={`text-3xl font-bold ${isMagical ? 'text-white' : 'text-slate-900'}`}>₾150.00</div>
            ...
            <div className="grid md:grid-cols-2 gap-10">
                ...
                <div>
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className={`text-2xl font-bold ${isMagical ? 'text-slate-100' : 'text-slate-900'}`}>
                            {isMagical ? text.magTitleMag : text.magTitleStd}
                        </h2>
                    </div>
                    <ul className="space-y-4 mb-8">
                        {magusFeatures.map((feat, i) => (
                            <li key={i} className={`flex items-start gap-3 ${isMagical ? 'text-slate-300' : 'text-slate-700'}`}>
                                <Check className={`shrink-0 mt-0.5 ${isMagical ? 'text-amber-500' : 'text-green-500'}`} size={18} />
                                <span>{feat}</span>
                            </li>
                        ))}
                    </ul>
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center
                                        ${selectedPeriod === opt.id 
                                            ? (isMagical ? 'border-amber-500' : 'border-blue-600') 
                                            : (isMagical ? 'border-slate-600' : 'border-slate-300')
                                        }
                                    `}>
                                        {selectedPeriod === opt.id && <div className={`w-2.5 h-2.5 rounded-full ${isMagical ? 'bg-amber-500' : 'bg-blue-600'}`} />}
                                    </div>
                                    <span className={`font-medium ${
                                        selectedPeriod === opt.id 
                                            ? (isMagical ? 'text-white' : 'text-blue-900') 
                                            : (isMagical ? 'text-slate-300' : 'text-slate-700')
                                    }`}>
                                        {opt.label}
                                    </span>
                                </div>
                                ...
                                <div className="text-right">
                                    <div className={`font-bold ${
                                        selectedPeriod === opt.id 
                                            ? (isMagical ? 'text-amber-400' : 'text-blue-700') 
                                            : (isMagical ? 'text-slate-200' : 'text-slate-900')
                                    }`}>
                                        ₾{opt.price}
                                    </div>
                                    {opt.savings && (
                                        <div className={`text-[10px] font-bold ${isMagical ? 'text-emerald-400' : 'text-green-600'}`}>
                                            {opt.savings}
                                        </div>
                                    )}
                                </div>
